...
Crawler.prototype = _.extend(Crawler.prototype, {
  ...
  shouldCrawl (url) {
    ...
    return _.findIndex(this.domains, function(domain) { return domain == parsedUrl.host; }) > -1
  },
    ...
    request.get(task.url, function (err, res, body) {
      ...
      self.emitter.emit('crawl', task.url, res, body, () => {
        ...
        if (res.headers['content-type'] && res.headers['content-type'].indexOf('html')) {
          ...
          $('a[href], link[href]').each(function(index, a) {
          ...
          $('img[src], script[src]').each(function(index, img) {
